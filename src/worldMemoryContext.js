...
function projectTownSummary(summary) {
  ...
  return {
    ...
    economy: projectTownMetricRecord(summary.economy, ['market', 'labor', 'build', 'caravan', 'wealth']),
    marketOutlook: asText(summary.marketOutlook, 40) || null,
    armory: projectTownMetricRecord(summary.armory, ['reserve', 'issued', 'repair', 'distribution']),
    armoryStatus: asText(summary.armoryStatus, 40) || null,
    gatePressure: Number.isFinite(Number(summary.gatePressure)) ? Number(summary.gatePressure) : null,
    gateStatus: asText(summary.gateStatus, 40) || null,
    gateCriticalEvent: asText(summary.gateCriticalEvent, 40) || null,
    gateTravelRisk: asText(summary.gateTravelRisk, 40) || null,
    gateLastEventDay: asNullableInteger(summary.gateLastEventDay),
    buildQueueDepth: Number(summary.buildQueueDepth) || 0,
    leadBuildType: asText(summary.leadBuildType, 40) || null,
    leadBuildPriority: asText(summary.leadBuildPriority, 40) || null,
    leadBuildQueuedDay: summary.leadBuildQueuedDay == null ? null : asNullableInteger(summary.leadBuildQueuedDay),
    autonomy: projectTownAutonomySummary(summary.autonomy),
    frontlineStatusLabel: asText(summary.frontlineStatusLabel, 80) || null,
    frontlineScore: Number.isFinite(Number(summary.frontlineScore)) ? Number(summary.frontlineScore) : null,
    raidStatus: asText(summary.raidStatus, 24) || null,
    militiaStrength: Number.isFinite(Number(summary.militiaStrength)) ? Number(summary.militiaStrength) : null,
    rationLevel: Number.isFinite(Number(summary.rationLevel)) ? Number(summary.rationLevel) : null,
    supplyStrain: Number.isFinite(Number(summary.supplyStrain)) ? Number(summary.supplyStrain) : null,
    supplyOutlook: asText(summary.supplyOutlook, 40) || null,
    lastRaidOutcome: asText(summary.lastRaidOutcome, 40) || null,
    lastRaidDay: summary.lastRaidDay == null ? null : asNullableInteger(summary.lastRaidDay),
